...
</div>`}static get properties(){return{icon:String,dataName:{type:String,value:""},service:String,title:String,maxlength:Number,pattern:String,q:{type:String,value:""},dropdownValue:Object,rows:{value:[]},params:{type:Object,value:""}}}getValue(){const select=this.shadowRoot.querySelector("#input");this.dropdownValue=select.value;return this.dropdownValue}valid(validation){}invalid(validation){}mouseOver(){const normalSelect=this.shadowRoot.querySelector("#input"),options=normalSelect.querySelectorAll("option");//check first if options is greater than 10 inorder to prevent normal dropdown
if(10<options.length){normalSelect.addEventListener("mousedown",function(e){e.preventDefault()})}}optionCount(){const items=this.shadowRoot.querySelector("#input").querySelectorAll("option");10<items.length?this.showFilter():console.log("items is less than 10")}showFilter(){console.log("showing filter");const self=this,selectSection=this.shadowRoot.querySelector("#select-section");selectSection.classList.remove("select");//to remove bulma dropdown arrow
const slim=new SlimSelect({select:this.shadowRoot.querySelector("#input"),placeholder:this.e.name,allowDeselect:!0,showContent:"down",deselectLabel:"<span style=\"color: #1c5a6b; text-shadow: -0.5px -1px 8px #00c8ff;\">x</span>"});slim.open();//get slimSelect search input
let searchInput=this.shadowRoot.querySelector(".ss-search").querySelector("input");searchInput.addEventListener("click",function(event){event.stopPropagation()});//check whether options are > 50 to search backend
const items=this.shadowRoot.querySelector("#input").querySelectorAll("option");if(50<=items.length){searchInput.addEventListener("keyup",function(event){let searchText=searchInput.value,column="q";// this.deleteParamKeys(columns.concat(['q']), false);
self.deleteParamKeys(column,!1);// update new search query param
self.updateParams(column,searchText)})}}onLoadData(dsc){Logger.i.info("onLoadData Callback");Logger.i.info(this.rows.length)}}customElements.define(DropdownSelect.is,DropdownSelect);