import{html,shopProductsSectionTheme,SharedStyles,shopProductItemTheme,SHOP_STYLES,StaticPosBase}from"../../../../components/adaptive-ui.js";class StaticPos extends StaticPosBase{renderDefault(){return html`
        ${SHOP_STYLES}
...
<div class="section" >
  ...
  <button class="key--equal" @click="${this.addPadCart}">+</button>
        ...
        `}constructor(){super()}firstUpdated(changedProperties){super.firstUpdated(changedProperties)}_filterKeypad(rows){const filtered_items=[];for(let i=0;i<rows.length;i++){const item=rows[i];// institution name isn't specified in values
var group=item.group;if("Keypad"===group){this.qs("#btn-keypad").style.display="unset";this.itemPad=item}else{filtered_items.push(item)}}return filtered_items}showpad(){if(!this.is_pad){this.qs("#products_section").style.display="none";this.qs(".pad").style.display="block";this.is_pad=!0}else{this.qs("#products_section").style.display="flex";this.qs(".pad").style.display="none";this.is_pad=!1}}display(numb){var result=this.qs(".calculator__display").textContent;this.qs(".calculator__display").textContent=+result+""+numb}clear(){this.qs(".calculator__display").textContent=""}setCharAt(str,index,chr){if(index>str.length-1)return str;return str.substr(0,index)+chr+str.substr(index+1)}addPadCart(){var result=this.qs(".calculator__display").textContent,retrievedObject=localStorage.getItem("pos-cart");const items=JSON.parse(retrievedObject);if(0==result.charAt(0)){result=this.setCharAt(result,0,"")}if(0!==+result){// this.itemPad.price = Number(result);
const product=this._findProductItem(this.itemPad.id);// console.log(cartItem);
this.currentProduct=product;product.price=+result;let cartItem=this._findInCartItem(items,this.itemPad.id);// console.log(cartItem);
if(cartItem===void 0){cartItem={item:product,quantity:1};items.push(cartItem)}else{var index=items.indexOf(cartItem);items[index].item.price=+items[index].item.price+ +result}}localStorage.setItem("pos-cart",JSON.stringify(items));this.cartItems=items;this.clear()}_changeQuantity(productId){const value=this.qs("#chart_item_"+productId).value,cartItem=this._findCartItem(productId),index=this.cartItems.indexOf(cartItem);var group=cartItem.group;if("Keypad"!==group){this.cartItems[index].quantity=+value;this.cartItems=[...this.cartItems];// this.cartItems[index] = cartItem['quantity'] + 1;
}else{this.qs("#chart_item_"+productId).value=1;this.qs("#chart_item_"+productId).textContent=1;this.cartItems[index].quantity=1;this.cartItems=[...this.cartItems]}// this.set('cartItems.' + index + '.quantity', cartItem['quantity'] + 1);
localStorage.setItem("pos-cart",JSON.stringify(this.cartItems))}closeCartView(){this.qs("#cart-view").classList.remove("is-active")}openCartView(){this.qs("#cart-view").classList.add("is-active")}init(pElement,loader){super.init(pElement,loader)}}customElements.define(StaticPos.is,StaticPos);