...
<div class="column">
    <div class="field">
        ...
        `}getInput(){const picker=this.shadowRoot.querySelector("#input");return picker}getValue(){const pickerDate=this.getInput().value;// AsDate;//.getUTCDate();
if(pickerDate){return this.convertTime24to12(pickerDate)}}valid(Validation){this.shadowRoot.querySelector(".validation-info").style.display="none";// Revert general text content
this.shadowRoot.querySelector(".validation-info").textContent="Required"}invalid(validation){this.shadowRoot.querySelector(".validation-info").style.display="block";if(validation){this.shadowRoot.querySelector(".validation-info").textContent=validation}}firstUpdated(changedProperties){super.firstUpdated(changedProperties);if(this.required&&!this.e.defaultValue){this.getInput().value=format(new Date,"HH:mm")}if(this.e.defaultValue){this.getInput().value=this.convertTime12to24(this.e.defaultValue)}}}customElements.define(TimeElement.is,TimeElement);