...
if(q){var query=q.toLowerCase(),name=item[1].toLowerCase();if(-1!==name.indexOf(query)){filtered_items.push(item)}}else{filtered_items.push(item)}if(10<filtered_items.length){break}}return filtered_items}init(pElement,loader){super.init(pElement,loader)}}customElements.define(IndustriesCategories.is,IndustriesCategories);