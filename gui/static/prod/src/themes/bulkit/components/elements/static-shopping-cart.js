...
_removeCartItem(evt){const product_id=evt.currentTarget.cartItem,cartItem=this._findCartItem(product_id);var index=this.cartItems.indexOf(cartItem);if(-1<index){this.cartItems.splice(index,1);this.cartItems=[...this.cartItems];localStorage.setItem("shop-cart",JSON.stringify(this.cartItems))}this.dispatchEvent(new CustomEvent("change-cart-count",{bubbles:!0,composed:!0,detail:{}}))}firstUpdated(changedProperties){super.firstUpdated(changedProperties)}}customElements.define(StaticShoppingCart.is,StaticShoppingCart);