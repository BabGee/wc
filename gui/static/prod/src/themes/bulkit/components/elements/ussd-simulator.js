import{utilsMixin,serviceCallMixin,BaseElement,COMMAND_VCS_MENU,Logger,LitElement,html}from"../../../../components/adaptive-ui.js";const UssdSimulatorBase=class extends utilsMixin(serviceCallMixin(BaseElement)){static get is(){return"ussd-simulator"}makeRequest(params){params.chid=4;return new Promise((resolve,reject)=>{this.callServiceParams(this.service,params).then(res=>{Logger.i.info(res);const vcsMenuCommand=res.serviceCommands[COMMAND_VCS_MENU];resolve(vcsMenuCommand.response)},function(rejected){reject(rejected)}).catch(function(exception){Logger.i.error(exception);reject(exception)})})}init(pElement,loader){super.init(pElement,loader);this.service=pElement.service}};var ussdSimulator={UssdSimulatorBase:UssdSimulatorBase};const CONNECTION_BEG="BEG",CONNECTION_END="END",CONNECTION_CON="CON";class UssdSimulatorPreview extends LitElement{constructor(){super();this.state=CONNECTION_CON}static get properties(){return{menus:String,connection:String,state:String}}render(){return html`
        <style>
        *{
