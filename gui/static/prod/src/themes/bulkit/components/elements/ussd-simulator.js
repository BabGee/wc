<<<<<<< HEAD
import{LitElement,html,Logger,UssdSimulatorBase}from"../../../../components/adaptive-ui.js";const CONNECTION_BEG="BEG",CONNECTION_END="END",CONNECTION_CON="CON";class UssdSimulatorPreview extends LitElement{constructor(){super();this.state=CONNECTION_CON}static get properties(){return{menus:String,connection:String,state:String}}render(){return html`
=======
import{utilsMixin,BaseElement,Logger,httpMixin,LitElement,html}from"../../../../components/adaptive-ui.js";const UssdSimulatorBase=class extends utilsMixin(httpMixin(BaseElement)){static get is(){return"ussd-simulator"}makeRequest(params){return new Promise((resolve,reject)=>{this.call(this.service,params,!1).then(res=>{Logger.i.info(res);resolve(res)},function(rejected){reject(rejected)}).catch(function(exception){Logger.i.error(exception);reject(exception)})})}init(pElement,loader){super.init(pElement,loader);this.service=pElement.service}};var ussdSimulator={UssdSimulatorBase:UssdSimulatorBase};const CONNECTION_BEG="BEG",CONNECTION_END="END",CONNECTION_CON="CON";/* eslint max-len: ["error", { "ignoreTemplateLiterals": true }]*/class UssdSimulatorPreview extends LitElement{constructor(){super();this.state=CONNECTION_CON}static get properties(){return{menus:String,connection:String,state:String}}render(){return html`
>>>>>>> d6dfa267387a22108b29e08fe5c46fa0b0d3909e
    ...
    `}cancel(evt){evt.preventDefault();const event=new CustomEvent("cancel",{detail:{}});this.dispatchEvent(event)}// attach on enter key press event
submit(evt){evt.preventDefault();const selectionInput=this.shadowRoot.querySelector("#selectionInput"),selection=selectionInput.value;// get current input
// clear input
selectionInput.value="";// fire event
const event=new CustomEvent("submit",{detail:{selection:selection}});this.dispatchEvent(event)}updateDisplay(menuString){this.menus=menuString}setState(state){this.state=state}}customElements.define("ussd-simulator-preview",UssdSimulatorPreview);var simulatorPreview={CONNECTION_BEG:CONNECTION_BEG,CONNECTION_END:CONNECTION_END,CONNECTION_CON:CONNECTION_CON};class Nexus4 extends LitElement{constructor(props){super(props);this.isLoading=!1;this.state=0;this.ussdString="";// generate a random string of X chars
this.session="";this.sessionState=CONNECTION_BEG}static get properties(){return{session:String,msisdn:String,serviceCode:String,ussdString:String,sessionState:String,isLoading:Boolean,dialed:Boolean}}render(){return html`
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    `}/*	Pusher	*/pusher(evt){var num=evt.currentTarget.dataset.v;const display=this.shadowRoot.querySelector(".number-area .numbers");display.innerHTML+=""+num+""}/*	Delete */delete(evt){const display=this.shadowRoot.querySelector(".number-area .numbers");var numbers=display.textContent,numbers2=numbers.length;display.innerHTML=numbers.substr(0,numbers2-1)}makeid(length){for(var result="",characters="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",charactersLength=characters.length,i=0;i<length;i++){result+=characters.charAt(Math.floor(Math.random()*charactersLength))}return result}_callCode(){var code=this.shadowRoot.querySelector("#code");const codeV=code.textContent;// remove * at begining
...
<<<<<<< HEAD
        `}_init(){var phone=this.shadowRoot.querySelector("#phone");let msisdn=phone.value;if(msisdn.startsWith("+")){msisdn=msisdn.substring(1)}else if(msisdn.startsWith("0")){msisdn="254"+msisdn.substring(1)}this.msisdn=msisdn;this.state=1}serialize(obj){const str="?"+Object.keys(obj).reduce(function(a,k){a.push(k+"="+encodeURIComponent(obj[k]));return a},[]).join("&");return str}}window.customElements.define(UssdSimulator.is,UssdSimulator);export{simulatorPreview as $simulatorPreview,CONNECTION_BEG,CONNECTION_END,CONNECTION_CON};
=======
        `}_init(){// this.makeRequest();
var phone=this.shadowRoot.querySelector("#phone");let msisdn=phone.value;if(msisdn.startsWith("+")){msisdn=msisdn.substring(1)}else if(msisdn.startsWith("0")){msisdn="254"+msisdn.substring(1)}this.msisdn=msisdn;this.state=1}// TODO this is duplicated some other place
// should be a util
serialize(obj){const str="?"+Object.keys(obj).reduce(function(a,k){a.push(k+"="+encodeURIComponent(obj[k]));return a},[]).join("&");return str}}window.customElements.define(UssdSimulator.is,UssdSimulator);export{simulatorPreview as $simulatorPreview,ussdSimulator as $ussdSimulator,CONNECTION_BEG,CONNECTION_CON,CONNECTION_END,UssdSimulatorBase};
>>>>>>> d6dfa267387a22108b29e08fe5c46fa0b0d3909e
