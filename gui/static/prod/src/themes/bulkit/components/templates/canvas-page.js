import{html,BULMA_STYLES,LANDING_STYLES,VIEW_MODE_DIALOG,CanvasPageBase}from"../../../../components/adaptive-ui.js";import"./section-page.js";class CanvasPage extends CanvasPageBase{render(){// todo implement 404 tab to all pages
if(!this.interface){return html`
      ${BULMA_STYLES}            
        ${LANDING_STYLES} 
...
`)} 
`}<snack-bar id="snack-bar" ?active="${this._snackbarOpened}">${this._snackbarMessage}</snack-bar>`}constructor(){super()}scrollPage(evt){const index=evt.currentTarget.pageIndex;// var elementToFocus = document.getElementById(window.location.hash.slice(1));
...
static get properties(){return{title:String,name:String,logo:String,view:String,defaultColor:String,pages:Array,tab:Object,group:Object,toggle:Boolean}}toggleNav(){if(this.toggle){this.qs(".navbar-wrapper").classList.remove("navbar-light");this.qs(".navbar-wrapper").classList.remove("mobile-menu-dark");this.qs(".nav-toggle").classList.remove("is-active");this.qs(".nav-right").classList.remove("is-active");this.toggle=!1}else{this.qs(".navbar-wrapper").classList.add("navbar-light");this.qs(".navbar-wrapper").classList.add("mobile-menu-dark");this.qs(".nav-toggle").classList.add("is-active");this.qs(".nav-right").classList.add("is-active");this.toggle=!0}}stateChanged(state){super.stateChanged(state)}/**
       * Dialogs Back navigation, Pop dialogs' stack
       *
       * @param evt
       * @private
       */_viewList(evt){if(!this.dialogsStack.length){this.view="list";this.updateLocationHash()}else{const args=this.dialogsStack.pop();console.log(args);const dialog=this.qs("#dialog");dialog.payload=args[0];dialog.params=args[1];dialog.loading=!1}}}window.customElements.define("canvas-page",CanvasPage);