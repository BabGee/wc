...
import "../../../../../../node_modules/@polymer/paper-dropdown-menu/paper-dropdown-menu.js"; // import '@polymer/paper-icon-button/paper-icon-button';

import { LitElement, html, css } from "../../../../../../node_modules/lit-element/lit-element.js";
import { DatalistHeaderStyles } from "../../../styles/datalist-header.js";
import { DatasourceTableHeadStyles } from "./datalist-element-styles/datasource-table-head-css.js";
/* eslint max-len: ["error", { "ignoreTemplateLiterals": true }]*/

export class DataSourceTableHead extends LitElement {
  static get styles() {
    return [DatalistHeaderStyles, DatasourceTableHeadStyles, css`
        :host {
          display: block;
        }
      `];
  ...
  _dateFromFilter(evt) {// console.log('date from filter');
    // console.log(evt);
  ...
  _dateToFilter(evt) {
    ...
    const x = {
      'start_date': moment.unix(picker.dateFrom).format('YYYY-MM-DD'),
      'end_date': moment.unix(picker.dateTo).format('YYYY-MM-DD')
    };
    this.fire('filter-date-range', {
      range: x
    });
  }
  ...
  _handleKeyDownInput(event) {
    ...
    if (this.previousValue !== this.currentValue) {
      if (event.keyCode === 13) {
        this.fire('input-change-th-content', {
          column: this.column,
