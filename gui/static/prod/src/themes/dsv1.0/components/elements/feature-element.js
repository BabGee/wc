import{html,SectionPElementDsc}from"../../../../components/adaptive-ui.js";class FeatureElement extends SectionPElementDsc{constructor(){super()}render(){return html`
    ...
    `)}
        ...
        `}static get is(){return"feature-element"}static get properties(){return{}}openLink(event){let link=event.target.getAttribute("data-name");window.open(link,"_blank")}_getTitle(text){let[title,link]=text.split("|");return title}_getLink(text){let[title,link]=text.split("|");return link}}customElements.define(FeatureElement.is,FeatureElement);var featureElement={FeatureElement:FeatureElement};export{featureElement as $featureElement,FeatureElement};