      ...
      <div class="ss-main">
        ...
        <div class="ss-multi-selected ${this.contentOpen?"ss-open-below":""}" @click="${this._onSingleSelectContainerClick}">
          ...
          ${this.selected.length?html`
              ...
              `)}
          `:html`
            <span class="placeholder">Select ${this.title}s</span>
          `}
</div>
          <div class="ss-add">
...
<p id="warning-text" style="color:#ff3860;">${this.e.name} required</p>
    ...
    `}getValue(){return this.selected.join(",")}firstUpdated(changedProperties){super.firstUpdated(changedProperties);const self=this;this.loader.then(()=>{});document.addEventListener("click",function(e){if(!self.shadowRoot.contains(e.currentTarget)){self.close()}else{console.log(e.target);console.log(self.contains(e.target))}})}static get properties(){return{icon:String,title:String,contentOpen:{type:Boolean},options:{type:Array},selected:Array}}valid(){const warningText=this.shadowRoot.querySelector("#warning-text");warningText.style.display="none"}invalid(){const warningText=this.shadowRoot.querySelector("#warning-text");warningText.style.display="block"}getOption(value){return this.options.find(function(element){return element.value==value})}getSelections(){const ss=this.selected.map(s=>this.getOption(s));console.log(ss);return ss}selectOptions(){const options=[];for(var i=0;i<this.options.length;i++){const item=this.options[i],option={};option.value=item[0];option.label=item[1];option.selected=this.selected.includes(option.value);options.push(option)}return options}_onSingleSelectContainerClick(e){e.preventDefault();e.stopPropagation();this.contentOpen?this.close():this.open()}_onDeselectValueClick(e){e.stopPropagation();const item=e.currentTarget.dataset.value;var filteredAry=this.selected.filter(e=>e!=item);console.log(filteredAry);this.selected=filteredAry}_onSearchInputClick(evt){evt.stopPropagation()}_onSearchKeyUp(e){const searchValue=e.target.value;this.updateParams("q",searchValue)}_onSearchFocus(){this.open()}_onOptionClick(e){e.preventDefault();e.stopPropagation();const selected=e.currentTarget.dataset.value;console.log(selected);this.selected=Array.from(new Set([...this.selected,selected]))}open(){if(this.contentOpen){return}this.contentOpen=!0}close(){if(!this.contentOpen){return}this.contentOpen=!1}}window.customElements.define(TagInput.is,TagInput);