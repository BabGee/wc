...
<p id="warning-text" style="color:#ff3860;">${this.e.name} required</p>
    ...
    `}getValue(){return this.selected.join(",")}firstUpdated(changedProperties){super.firstUpdated(changedProperties);const self=this;this.loader.then(dsc=>{});// TODO #307 Test whether this event listener is automatically detached when this component is detached
document.addEventListener("click",function(e){// TODO this doesn't work
if(!self.shadowRoot.contains(e.currentTarget)){self.close()}else{console.log(e.target);console.log(self.contains(e.target))}})}static get properties(){return{icon:String,title:String,// slim
contentOpen:{type:Boolean},options:{type:Array},selected:Array}}valid(validation){const warningText=this.shadowRoot.querySelector("#warning-text");warningText.style.display="none"}invalid(validation){const warningText=this.shadowRoot.querySelector("#warning-text");warningText.style.display="block"}getOption(value){const o=this.rows.find(function(element){return element[0]==value});if(o){return{label:o[1],value:o[0]}}}// used for multi select
getSelections(){const ss=this.selected.map(s=>this.getOption(s));console.log(ss);return ss}/**
     * adds a property 'selected=true' to options that have been selected
     *
     * @return {Array}
     */selectOptions(){const options=[];for(var i=0;i<this.rows.length;i++){const item=this.rows[i],option={};// first item is the pk
// second item is the label
option.value=item[0];option.label=item[1];option.selected=this.selected.includes(option.value);options.push(option)}return options}_onSingleSelectContainerClick(e){e.preventDefault();e.stopPropagation();this.contentOpen?this.close():this.open()}_onDeselectValueClick(e){e.stopPropagation();const item=e.currentTarget.dataset.value;// remove animation is set by adding class ss-out
// TODO e.currentTarget.parentElement.classList.add('ss-out');
// remove item from selections
var filteredAry=this.selected.filter(e=>e!=item);console.log(filteredAry);this.selected=filteredAry}_onSearchInputClick(evt){evt.stopPropagation()}_onSearchKeyUp(e){const searchValue=e.target.value;this.updateParams("q",searchValue)}_onSearchFocus(e){// todo slim.js 342
...
}open(){if(this.contentOpen){return}this.contentOpen=!0;// if (_this.config.searchFocus) {
