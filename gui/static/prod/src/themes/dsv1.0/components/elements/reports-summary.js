import{html,ReportsSummaryBase}from"../../../../components/adaptive-ui.js";class ReportsSummary extends ReportsSummaryBase{renderDefault(){return html`
        ...
        `}constructor(){super()}firstUpdated(changedProperties){super.firstUpdated(changedProperties)}drawChart(){const self=this;google.charts.load("current",{packages:["corechart"]});google.charts.setOnLoadCallback(drawChart2);function drawChart2(){var data=google.visualization.arrayToDataTable([["Year","Sales","Expenses"],["2004",1e3,400],["2005",1170,460],["2006",660,1120],["2007",1030,540]]// Date, Institution1, Institution2, Institution3
// 2004, 10,            10,          10
// 2005, 20,            20,          20
// 2006, 30             30,          30
),options={title:"Prices Chart",curveType:"function",legend:{position:"bottom"}},chart=new google.visualization.LineChart(self.shadowRoot.querySelector("#curve_chart"));chart.draw(data,options)}}// todo override kept because of qs
generatePDF(){const self=this,autoPrint=!1;var doc=new jsPDF("p","mm","a3");const printer=new Printer(doc);// add a margin on top
printer.moveY(5);// print bid name
printer.moveX(60);printer.printText("Bid Details",Printer.BOLD);printer.moveY(20);printer.printText("Bid Name: "+this.bidName,Printer.NORMAL);printer.moveY(20);printer.printText("Bid Opening Date: "+this.bidOpeningDate,Printer.NORMAL);printer.moveY(20);printer.printText("Bid Closing Date: "+this.bidClosingDate,Printer.NORMAL);printer.moveY(20);printer.printText("Bid Opening Price: "+this.bidOpeningPrice,Printer.NORMAL);printer.moveY(20);printer.printText("Bid Closing Price: "+this.bidClosingPrice,Printer.NORMAL);if(this.buyer){printer.moveY(20);printer.printText("Bid Participants ",Printer.BOLD);for(let i=0;i<this.participants.length;i++){const participant=this.participants[i];printer.moveY(10);printer.printText("Participant Name: "+participant.name,Printer.NORMAL);printer.moveY(10);printer.printText("Opening price: "+participant.openingPrice,Printer.NORMAL);printer.moveY(10);printer.printText("Closing price: "+participant.closingPrice,Printer.NORMAL);printer.moveY(10)}printer.moveY(20);printer.printText("Auction Summary",Printer.BOLD);printer.moveY(20);printer.printText("Winning Bid: "+this.bidClosingPrice,Printer.NORMAL);printer.moveY(20);printer.printText("Winning Supplier: "+this.closingBidder,Printer.NORMAL);printer.moveY(20);printer.printText("Saving Amount: "+this._computeSavingAmount(this.bidOpeningPrice,this.bidClosingPrice),Printer.NORMAL);printer.moveY(20);printer.printText("Service Charges: "+this._serviceCharge(this.bidOpeningPrice,this.bidClosingPrice),Printer.NORMAL)}else{printer.moveY(20);printer.printText("Bid Price Changes ",Printer.BOLD);for(let i=0;i<this.priceChanges.length;i++){const participant=this.priceChanges[i];printer.moveY(10);printer.printText("Price: "+participant.price,Printer.NORMAL);printer.moveY(10);printer.printText("Date: "+participant.date,Printer.NORMAL);printer.moveY(10)}}if(autoPrint){doc.autoPrint();var iframe=this.$.iframe;iframe.src=doc.output("dataurlstring")}else{doc.save("report.pdf")}}}customElements.define(ReportsSummary.is,ReportsSummary);