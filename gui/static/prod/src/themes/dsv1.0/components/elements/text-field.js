import{utilsMixin,SerializableElement,html}from"../../../../components/adaptive-ui.js";const TextFieldBase=class extends utilsMixin(SerializableElement){static get is(){return"text-field"}static get properties(){return{params:{type:Object,value:{}},type:String,value:String,columnSize:{type:Array}}}getName(){return this.e.formName}validate(){if((this.e.required||this.required)&&!this.getValue()){return new this.Validation(!1,this.e.name+" is Required")}return new this.Validation(!0,"valid")}valid(){}invalid(){}firstUpdated(changedProperties){super.firstUpdated(changedProperties)}init(pElement,loader){super.init(pElement,loader);const self=this;self.title=TextFieldBase.toTitleCase(pElement.name);this.required=this.e.required||pElement.min&&0<pElement.min;this.value=pElement.defaultValue||""}};var textField={TextFieldBase:TextFieldBase};class TextField extends TextFieldBase{renderDefault(){return html`
        <style>
            .input-field{
                display:flex;
                padding-bottom:13px;
