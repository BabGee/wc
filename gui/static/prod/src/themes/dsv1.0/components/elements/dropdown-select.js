...
const slim=new SlimSelect({select:this.shadowRoot.querySelector("#select"),placeholder:this.e.name,showContent:"down",onChange:info=>{selectDiv.className="select is-hidden";//adding bulma back
arrow.classList.remove("is-hidden");selected.className="";selected.innerHTML=info.text;this.selectedIndex=select.value;self.deleteParamKeys("q",!0);slim.destroy()}});slim.open();//slim select search input from closing dropdown
