import{LitElement,html,css}from"../../../../../components/adaptive-ui.js";class TableTypeFooter extends LitElement{render(){return html`
        <style>
            :host {
                display: block;
            }
        ...
        </div>`}constructor(){super();this.availableSize=[];this.size=50}static get is(){return"table-type-footer"}static get properties(){return{footerPosition:String,size:{type:Number},page:{type:Number},totalElements:{type:Number},totalPages:{type:Number},availableSize:Array}}_computeCurrentSize(page,size){return(page-1)*size+1}_computeCurrentMaxSize(page,size,totalElements){const maxSize=size*page;return maxSize>totalElements?totalElements:maxSize}_nextPage(){if(this.page<this.totalPages){this.page=this.page+1}this.dispatchEvent(new CustomEvent("n-page",{detail:{page:this.page}}))}_prevPage(){if(0<this.page-1){this.page=this.page-1}this.dispatchEvent(new CustomEvent("p-page",{detail:{page:this.page}}))}_nextButtonEnabled(page,totalPages){return page<totalPages}_prevButtonEnabled(page){return 1<page}_newSizeIsSelected(){const newSize=this.shadowRoot.querySelector("paper-listbox").selected;if(newSize){if(null!==this.oldPage&&this.oldPage!==void 0){this.page=1}this.size=newSize;this.dispatchEvent(new CustomEvent("size-change",{detail:{size:newSize}}))}}_computePosition(position){if("right"===position){return"end-justified"}return""}}customElements.define(TableTypeFooter.is,TableTypeFooter);var tableTypeFooter={TableTypeFooter:TableTypeFooter};const TableStyles=css`
...
`;var tableType={TableStyles:TableStyles};class TableTypeHeader extends LitElement{static get styles(){return[TableStyles,css`
        :host {
          display: block;
        }
      `]}render(){return html`
    ...
    <div class="sub-container">
      ...
      <paper-input style="display: inline-block"
                   ...
                   id="q"></paper-input>
        <button class="button" style="background-color:var(--app-primary-color); color: #fff; padding: 8px .75em;" type="submit" @click="${this._search}">Search</button>
        <button class="button" style="background-color:var(--app-primary-color); color: #fff; padding: 8px .75em;" type="button" @click="${this._searchReset}">Clear</button>
...
`}static get is(){return"table-type-header"}static get properties(){return{title:String,columns:{type:Array// value: () => [],
// notify: true,
//   language: String,
//   column: {
//     type: Object,
...
this.dispatchEvent(new CustomEvent("export-type",{detail:{type:pdf}}))}searchFields(columns){return columns.filter(function(item){return item.filter})}_searchReset(evt){const self=this;this.dispatchEvent(new CustomEvent("_clearSearch",{detail:{searchFields:self.searchFields(self.columns).map(function(field){return field.propertyPath})}}))}_searchFieldsExist(columns){return 0<columns.filter(function(item){return item.filter}).length}_search(evt){const self=this,q=self.shadowRoot.querySelector("#q").value,qIn=self.shadowRoot.querySelector("#qIn").selected;// console.log(q);
this.searchText=q;if(q){// console.log(self.qIn);
// self.q = q;
// console.log(evt);
this.dispatchEvent(new CustomEvent("_search",{detail:{column:qIn,searchFields:self.searchFields(self.columns).map(function(field){return field.propertyPath}),value:q}}))}}}customElements.define(TableTypeHeader.is,TableTypeHeader);var tableTypeHeader={TableTypeHeader:TableTypeHeader};class TableType extends LitElement{constructor(){super();this.columns=[];this.cols=[];this.data=[];this.selected=[];this.availableSize=[10,50,100,500]}static get styles(){return[TableStyles,css`
            :host {
              display: block;
            }
          `]}render(){return html`
...
// }
}_searchReset(evt){this._clearSearch()}_search(evt){var filter=evt.detail.value,column=evt.detail.column,columns=evt.detail.searchFields;this.dispatchEvent(new CustomEvent("search",{detail:{column:column,searchFields:columns,value:filter}}))}_clearSearch(event){var columns=event.detail.searchFields;this.dispatchEvent(new CustomEvent("clear-search",{detail:{searchFields:columns}}))}}customElements.define(TableType.is,TableType);var tableType$1={TableType:TableType};export{tableType$1 as $tableType,tableType as $tableType$1,tableTypeFooter as $tableTypeFooter,tableTypeHeader as $tableTypeHeader,TableStyles,TableType,TableTypeFooter,TableTypeHeader};