...
// self.q = q;
// console.log(evt);
this.dispatchEvent(new CustomEvent("search",{detail:{column:qIn,searchFields:self.searchFields(self.columns).map(function(field){return field.propertyPath}),value:q}}))}}_clearSearch(event){const self=this;this.dispatchEvent(new CustomEvent("clear-search",{detail:{searchFields:self.searchFields(self.columns).map(function(field){return field.propertyPath})}}))}}customElements.define(TableType.is,TableType);