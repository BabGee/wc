...
<div class="column is-8 is-block is-paddingless">
        <div class="field">
          <div class="select-country">
           ...
           ${this.loading?html`<loader-element></loader-element>`:html`${this.rows.map(row=>html`<option ?selected="${"254"===row[0]}" value="${row[0]}">${row[1]} +${row[0]}</option>`)}`} 
            </select>
              </div>
...
this.shadowRoot.querySelector(".info-error").textContent="Required"}invalid(validation){this.shadowRoot.querySelector(".info-error").style.display="block";if(validation){this.shadowRoot.querySelector(".info-error").textContent=validation}}}customElements.define(MsisdnInput.is,MsisdnInput);