...
export const ForceDirectedBase = class extends utilsMixin(dataSourceMixin(SerializableElement)) {
  ...
  initForceDirected() {
    ...
    var x_scale = d3.scale.linear().domain([0, 1]).range([x_center, x_center + radius]);
    var y_scale = d3.scale.linear().domain([0, 1]).range([y_center, y_center + radius]);
    ...
    d3.json('data.json', function (error, graph) {
      ...
      var max_value = d3.max(graph.links, function (d) {
        return d.value;
      });
      var link = svg.selectAll('.link').data(graph.links).enter().append('line').attr('class', 'link').style('opacity', function (d) {
    ...
    d3.selectAll('#controls input[name=mode]').on('change', function () {
      if (this.value == 'circle') {
        ...
        svg.selectAll('line').transition().duration(1000).attr('x1', function (d) {
          return x_scale(Math.sin(index_to_rad(d.source.index)));
        }).attr('x2', function (d) {
          return x_scale(Math.sin(index_to_rad(d.target.index)));
        }).attr('y1', function (d) {
          return y_scale(Math.cos(index_to_rad(d.source.index)));
        }).attr('y2', function (d) {
          return y_scale(Math.cos(index_to_rad(d.target.index)));
        });
        svg.selectAll('circle').transition().duration(1000).attr('cx', function (d, i) {
          return x_scale(Math.sin(index_to_rad(i)));
        }).attr('cy', function (d, i) {
          return y_scale(Math.cos(index_to_rad(i)));
        });
      } else {
        svg.selectAll('line').transition().duration(1000).attr('x1', function (d) {
          return d.source.x_resume;
        }).attr('y1', function (d) {
          return d.source.y_resume;
        }).attr('x2', function (d) {
          return d.target.x_resume;
        }).attr('y2', function (d) {
          return d.target.y_resume;
        });
        svg.selectAll('circle').transition().duration(1000).attr('cx', function (d) {
          return d.x_resume;
        }).attr('cy', function (d) {
          return d.y_resume;
        });
