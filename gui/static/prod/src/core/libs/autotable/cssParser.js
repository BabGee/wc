...
import { marginOrPadding } from "./common.js";
...
export function parseCss(element, scaleFactor, ignored = []) {
  ...
  assign('cellPadding', parsePadding(style.padding, style.fontSize, style.lineHeight, scaleFactor)); // style.borderWidth only works in chrome (borderTopWidth etc works in firefox and ie as well)
...
function parseColor(element, colorProp) {
  const cssColor = realColor(element, colorProp);
...
function parsePadding(val, fontSize, lineHeight, scaleFactor) {
  ...
  const linePadding = (parseInt(lineHeight) - parseInt(fontSize)) / scaleFactor / 2;
