...
export function getStringWidth(text, styles) {
  ...
  const widestLineWidth = textArr.map(text => state().doc.getTextWidth(text)) // Shave off a few digits for potential improvement in width calculation
  .map(val => Math.floor(val * 10000) / 10000).reduce((a, b) => Math.max(a, b), 0);
