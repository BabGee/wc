...
export const StaticProductsScrollBase = class extends utilsMixin(dataSourceMixin(BaseElement)) {
  constructor() {
    super();
  ...
  firstUpdated(changedProperties) {
    ...
    this.loader.then(dsc => {
      ...
      self.currentData = self.data; // self.selectedProduct =self.productItems[0];
    }); // self.selectedProduct ={};
    // self.selectedProduct.image ="";
  }

  static get observers() {
    return ['_groupItems(productItems.splices,search,business)'];
  }
  ...
  _computeBusinesses(items) {
    ...
    for (let i = 0; i < self.productItems.length; i++) {
      const business = self.productItems[i]['institution_name']; // institution name isn't specified in values
  ...
  _groupItems() {
    ...
    for (var i = 0; i < self.productItems.length; i++) {
      ...
      if (self.search) {
        ...
        var name = item['name'].toLowerCase(); // TODO var barcode = item['barcode'];

        if (name.indexOf(query) !== -1) {
          // || TODO barcode.indexOf(string) !== -1
          row.push(item);
        }
      } else {
        row.push(item);
  ...
  init(pElement, loader) {
    ...
    self.title = StaticProductsScrollBase.toTitleCase(pElement.name);
