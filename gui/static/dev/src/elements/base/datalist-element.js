import { mqttMixin } from "../../core/mixins/mqtt-mixin.js";
...
import "../../../node_modules/jspdf/dist/jspdf.min.js"; // import 'jspdf-autotable';

import { default as AutoTable } from "../../core/libs/autotable/main.js";
...
export const DataListElementBase = class extends utilsMixin(dataSourceMixin(mqttMixin(SerializableElement))) {
  ...
  _handleFilter(event) {
    ...
    var column = event.detail.column;
    this.updateParams(column.propertyPath, filter); // var eventRow = document.createTextNode(filter + ' is filtered on ' + column + ' column');
  ...
  _processTemplateLiteral(literal) {
    ...
    const tokens = columns2[1].substring(1); // console.log(column,extreme,tokens);
  ...
  generateCSV() {
    ...
    if ('export' in this.e.details) {
      // TODO verify properties existence
  ...
  generatePDF() {
    const self = this; // Only pt supported (not mm or in)
  ...
  onMqttMessage(message) {
    ...
    self.parseResponseIntoProperties(payload); // self.$.datasource._parsePayload(payload);
  }
