import { Row, Cell, Column, Table } from "./models.js";
import { getTheme, defaultConfig, defaultStyles } from "./config.js";
import { parseHtml } from "./htmlParser.js";
import { assign } from "./polyfills.js";
import { getStringWidth, marginOrPadding } from "./common.js";
import state, { getGlobalOptions, getDocumentOptions } from "./state.js";
...
function parseContent(table) {
      ...
      for (const column of table.columns) {
        if (rowSpansLeftForColumn[column.index] == null || rowSpansLeftForColumn[column.index].left === 0) {
          if (columnSpansLeft === 0) {
            ...
            const cell = new Cell(rawCell, styles, sectionName); // dataKey is not used internally anymore but keep for backwards compat in hooks
  ...
  table.allRows().forEach(row => {
    for (const column of table.columns) {
      const cell = row.cells[column.index]; // For now we ignore the minWidth and wrappedWidth of colspan cells when calculating colspan widths.
...
function getTableColumns(settings) {
  if (settings.columns) {
    ...
    const columns = [];
    Object.keys(firstRow).filter(key => key !== '_element').forEach(key => {
