...
export class DataSource extends Command {
  ...
  _parsePayload(payload, responseType) {
    const self = this;
    self.cols = payload.cols; // todo does this also only exists for `DATA` response type
    ...
    if (responseType === Symbol.for('LIST')) {
      ...
      for (var i = 0; i < self.rows.length; i++) {
        ...
        for (var j = 0; j < self.cols.length; j++) {
          ...
          if (col['type'] === 'href') {
            if ('links' in col) {
              ...
              linkProcessed['params'] = this.linkParams(linkParts['params'], item);
        ...
        item['links'] = itemLinks; // Ensure we have an id property
      ...
      self.data = items; // console.log(JSON.stringify(self.data));
      // TODO move_columns generation into datalist
      // console.log(self.data);

      const columns = [];
      let processedActions = false; // add selectable column
      ...
      self.cols.forEach(function (col) {
        ...
        if (col['type'] === 'href') {
          ...
          datalistColumn.choices = datalistColumn.date ? 'date_filters' in col && col['date_filters'] : 'list_filters' in col && col['list_filters']; // list_filters
          // console.log(datalistColumn);

          columns.push(datalistColumn);
        }
      });
      self.columns = columns; // console.log(self.columns)
