...
import { Redirect } from "./commands/redirect.js";
import { GetSection } from "./commands/get-section.js";
...
export const COMMAND_CLEAR_CART = 'clear_cart'; // payload['response_status'] === '96' todo handle known response errors with appropriate messages
...
export class Response {
  ...
  parse(serviceCommandName, config, cache) {
    ...
    const serviceCommandResponse = this.response['response'][serviceCommandName]; // 2. pass into appropriate parser
    ...
    switch (serviceCommandName) {
      ...
      default:
        console.debug(`using default command Parser for ${serviceCommandName}`);
