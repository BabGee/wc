...
import { Paho } from "../libs/paho-mqtt.js"; // called when the client connects
...
class Mqtt {
  setUp() {
    ...
    const port = 443;
    const path = '/ws';
    const clientId = 'myclientid_' + parseInt(Math.random() * 10000, 10); // Create a client instance

    const client = new Paho.MQTT.Client(host, port, path, clientId); // Connect Options
    ...
    window.MQTTclient.p_registers = []; // set callback handlers

    client.onConnectionLost = self._onConnectionLost;
    client.onMessageArrived = self._onMessageArrived; // connect the client
  ...
  _onConnectionLost(responseObject) {
    ...
    console.log('onConnectionLost:' + responseObject.errorMessage); // this._connect();
  ...
  connect() {
    console.log(JSON.stringify(this.options)); // Using the HiveMQ public Broker, with a random client Id

    this.client.connect(this.options); // Creates a new Messaging.Message Object and sends it to the HiveMQ MQTT Broker
  }
...
export const mqttMixin = BaseClass => class extends BaseClass {
  constructor() {
    super();
    const mqtt = new Mqtt();
    const c = mqtt.setUp(); // console.log(c);
