...
export const NavbarDefaultBase = class extends utilsMixin(serviceCallMixin(BaseElement)) {
  ...
  parseMenus() {
    const menuGroups = []; // todo perfoming parsing of the response
    ...
    for (const key in menus) {
      ...
      if (menus.hasOwnProperty(key)) {
        ...
        const title = Object.keys(menu)[0]; // TODO title length cannot be > 1

        const menuGroup = {
          'title': title,
          'items': []
        };
        const menuItemsWrapper = menu[title]; // menu items

        const itemsOrder = Object.keys(menuItemsWrapper); // todo sort order
  ...
  loadService(service, navigateTo) {
    if (window.preloads[navigateTo] && window.preloads[navigateTo]['res']) {
      ...
      this.callService(service).then(res => {
        // res.parseAll({});
        window.preloads[navigateTo] = {
          'res': res.serviceCommands['get_section'],
          'service': service
        }; // if already navigated, switch to section
  ...
  init(pElement, loader) {
    ...
    var self = this; // self.title = NavbarDefaultBase.toTitleCase(pElement.name);
