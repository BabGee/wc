...
export const InvoiceElementBase = class extends utilsMixin(dataSourceMixin(BaseElement)) {
  constructor() {
    super();

    const rP = document.querySelector('adaptive-ui')._payload;
        ...
        for (var j = 0; j < dsc.cols.length; j++) {
          ...
          if (dsc.cols[j]['type'] === 'href') {
            ...
            for (var link in links) {
              var linkObject = links[link]; // console.log(link + " -> " + linkObject);
              ...
              for (var linkParamKey in linkObject['params']) {
                var linkParam = linkObject['params'][linkParamKey]; // get from item, simple hack since links are the always the last

                linkProcessed['params'][linkParamKey] = item[linkParam];
              }

              itemLinks.push(linkProcessed); // console.log(item);
  ...
  printPdf(autoPrint) {
    ...
    doc.text(X + GAP, Y, 'INVOICE #' + self.invoice); // print payer
  ...
  dottedLine(doc, xFrom, yFrom, xTo, yTo, segmentLength) {
    ...
    var c = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2)); // Make sure we have an odd number of line segments (drawn or blank)
    // to fit it nicely

    var fractions = c / segmentLength;
    var adjustedSegmentLength = Math.floor(fractions) % 2 === 0 ? c / Math.ceil(fractions) : c / Math.floor(fractions); // Calculate x, y deltas per segment
