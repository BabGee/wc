...
export const StaticPosBase = class extends utilsMixin(dataSourceMixin(BaseElement)) {
  ...
  _computeTotals() {
    ...
    return this.cartItems.reduce(function (accumulator, currentValue, currentIndex, array) {
  ...
  _computeQuantity() {
    ...
    this.pl.bind = this.cartItems.reduce(function (accumulator, currentValue, currentIndex, array) {
      return accumulator + currentValue.quantity;
    }, 0);
  }

  _findCartItem(productId) {
    var results = this.cartItems.filter(function (item) {
  ...
  _addCartItem(item) {
    ...
    let cartItem = this._findInCartItem(items, productId); // console.log(cartItem);
    ...
    if (cartItem === undefined) {
      ...
      items[index]['quantity'] = Number(items[index]['quantity']) + 1; // this.set('cartItems.'+index+'.quantity',Number(cartItem['quantity']) + quantity);
