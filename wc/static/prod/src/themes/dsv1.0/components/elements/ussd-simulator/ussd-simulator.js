import{utilsMixin,BaseElement,Logger,httpMixin,css,LitElement,html}from"../../../../../components/adaptive-ui.js";const UssdSimulatorBase=class extends utilsMixin(httpMixin(BaseElement)){static get is(){return"ussd-simulator"}makeRequest(params){return new Promise((resolve,reject)=>{this.call(this.service,params,!1).then(res=>{Logger.i.info(res);resolve(res)},function(rejected){reject(rejected)}).catch(function(exception){Logger.i.error(exception);reject(exception)})})}init(pElement,loader){super.init(pElement,loader);this.service=pElement.service}};var ussdSimulator={UssdSimulatorBase:UssdSimulatorBase};const PhoneStyles=css`
* {
    margin: 0;
    padding: 0;
}
...
`;var phoneCss={PhoneStyles:PhoneStyles};const SimulatorPreviewStyles=css`
...
`;var simulatorPreviewCss={SimulatorPreviewStyles:SimulatorPreviewStyles};const CONNECTION_BEG="BEG",CONNECTION_END="END",CONNECTION_CON="CON";class UssdSimulatorPreview extends LitElement{constructor(){super();this.state=CONNECTION_CON}static get properties(){return{menus:String,connection:String,state:String}}static get styles(){return[SimulatorPreviewStyles,css`
        :host {
          display: block;
        }
      `]}render(){return html`
