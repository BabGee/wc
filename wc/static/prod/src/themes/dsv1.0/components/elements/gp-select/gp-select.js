import{dataSourceMixin,utilsMixin,SerializableElement,html,css}from"../../../../../components/adaptive-ui.js";const GpSelectBase=class extends utilsMixin(dataSourceMixin(SerializableElement)){static get is(){return"gp-select"}static get properties(){return{title:String,selected:Object,icon:String,maxlength:Number,pattern:String,q:String,dropdownValue:Object,params:Object,columnSize:Array}}getValue(){return this.selected}getName(){const el=this.e.formName;return el}validate(){if(this.e.required&&!this.getValue()){return!1}return!0}_dataJoined(data){const self=this;var concat_exclude=[];if(self.e.details.concat_exclude){concat_exclude=self.e.details.concat_exclude.split(",")}return data.filter(function(item,index){return!(0===index||concat_exclude.includes(index+"")||self.cols.length&&"href"===self.cols[index].type)}).join(" ")}dscDataName(){return this.e.defaultValue}firstUpdated(changedProperties){super.firstUpdated(changedProperties);const self=this;this.loader.then(()=>{self.q="";self.currentGroups=self.groups;self.currentData=self.data;if(0<Object.keys(self.rows).length&&!self.dropdownValue){self.dropdownValue=self.rows[0][0]}})}_computeItemsCount(rows){return rows.length}_shouldFilter(items){var b=this._computeItemsCount(items);return 10<b}_computeItems(rows,q){const filtered_items=[];for(let i=0;i<rows.length;i++){const item=rows[i];if(q){var query=q.toLowerCase(),name=item[1].toLowerCase();if(-1!==name.indexOf(query)){filtered_items.push(item)}}else{filtered_items.push(item)}}return filtered_items}init(pElement,loader){super.init(pElement,loader);var self=this;self.required=this.e.required||pElement.min&&0<pElement.min;self.title=GpSelectBase.toTitleCase(pElement.name);self.icon=pElement.icon;self.params=self.pl.paramsCopy();self.maxlength=pElement.max;self.loader=this.loadData()}};var gpSelect={GpSelectBase:GpSelectBase};const template=html`
...
`;var gpSelectCss={GpSelectStyles:GpSelectStyles};class GpSelect extends GpSelectBase{static get styles(){return[GpSelectStyles,css`
        :host {
          display: block;
        }
      `]}constructor(){super()}static get properties(){return{icon:String,dataName:{type:String,value:""},service:String,title:String,maxlength:Number,pattern:String,q:{type:String,value:""},dropdownValue:Object,rows:{value:[]},params:{type:Object,value:""},columnSize:{type:Array}}}renderDefault(){return html`
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bulma/0.7.5/css/bulma.css" type="text/css"/>
      <style>
        ...
        `}dropdown(link,id){if(null==link||link==void 0){this.shadowRoot.querySelector(".account-body").classList.toggle("is-block")}else{const headTitle=this.shadowRoot.querySelector(".account-header").querySelector("p"),allLists=this.shadowRoot.querySelector(".account-body").querySelectorAll("li");headTitle.innerHTML=`${link}`;allLists.forEach(list=>list.className="");this.shadowRoot.querySelector("#list-"+id).className="list-active";this.shadowRoot.querySelector(".account-body").classList.toggle("is-block")}}firstUpdated(changedProperties){super.firstUpdated(changedProperties)}}customElements.define(GpSelect.is,GpSelect);export{gpSelect as $gpSelect,gpSelectCss as $gpSelectCss,GpSelectBase,GpSelectStyles};