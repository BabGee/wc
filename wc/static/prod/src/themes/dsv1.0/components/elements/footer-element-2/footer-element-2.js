import{css,html,SectionPElementDsc}from"../../../../../components/adaptive-ui.js";const FooterElement2Styles=css`
body{
...
`;var footerElementCss2={FooterElement2Styles:FooterElement2Styles};class FooterElement2 extends SectionPElementDsc{constructor(){super()}static get styles(){return[FooterElement2Styles,css`
            :host {
              display: block;
            }
          `]}render(){return html`
        ...
        `}static get is(){return"footer-element-2"}init(pElement,loader){super.init(pElement,loader)}checkLink(e){const link=e.target.getAttribute("href");if(""==link){e.preventDefault()}}_getCurrentYear(){let year=new Date().getFullYear();return year}_getHref(text){if(text.includes("|")){let[title,link]=text.split("|");return link}else{return""}}_getLinkTitle(text){if(text.includes("|")){let[title,link]=text.split("|");return title}else{return text}}firstUpdated(changedProperties){super.firstUpdated(changedProperties)}}customElements.define(FooterElement2.is,FooterElement2);export{footerElementCss2 as $footerElementCss$2,FooterElement2Styles};